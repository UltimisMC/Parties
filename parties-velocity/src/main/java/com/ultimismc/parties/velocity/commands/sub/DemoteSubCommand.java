...
public class DemoteSubCommand implements SubCommand {
    ...
    public void execute(CommandSource source, String[] args) {
        ...
        if (args.length != 1) {
            player.sendMessage(
                    LegacyComponentSerializer.legacyAmpersand().deserialize(
                            "&9&l[PARTY] &cUsage: /party demote <player>"
                    )
            );
            return;
        }
        ...
        PartyRole newRole = partyTarget.getRole().getPrevious();
        if (newRole == partyTarget.getRole()) {
            player.sendMessage(
                    LegacyComponentSerializer.legacyAmpersand().deserialize(
                            "&9&l[PARTY] &cThis player already have the lowest rank."
                    )
            );
            return;
        }
            ...
            if (party.isMember(online.getUniqueId())) {
                online.sendMessage(LegacyComponentSerializer.legacyAmpersand().deserialize(
                        "&9&l[PARTY] " + Utils.getColor(target.getUniqueId()) + target.getUsername()
                                + "&f got demoted to &e" + newRole.name().charAt(0) + newRole.name().substring(1).toLowerCase()
